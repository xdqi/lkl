...
static inline int ifindex_to_name(int sock, struct lkl_ifreq *ifr, int ifindex)
{
	...
	return lkl_sys_ioctl(sock, LKL_SIOCGIFNAME, (__lkl_long_t)ifr);
}
...
int lkl_ifname_to_ifindex(const char *name)
{
	...
	ret = lkl_sys_ioctl(fd, LKL_SIOCGIFINDEX, (__lkl_long_t)&ifr);
	if (ret < 0)
		return ret;
...
int lkl_if_up(int ifindex)
{
	...
	err = lkl_sys_ioctl(sock, LKL_SIOCGIFFLAGS, (__lkl_long_t)&ifr);
	if (!err) {
		ifr.lkl_ifr_flags |= LKL_IFF_UP;
		err = lkl_sys_ioctl(sock, LKL_SIOCSIFFLAGS, (__lkl_long_t)&ifr);
	}
...
int lkl_if_set_mtu(int ifindex, int mtu)
{
	...
	err = lkl_sys_ioctl(sock, LKL_SIOCSIFMTU, (__lkl_long_t)&ifr);
...
int lkl_if_set_mac(int ifindex, void *addr)
{
	...
	err = lkl_sys_ioctl(sock, LKL_SIOCGIFHWADDR, (__lkl_long_t)&ifr);
	if (!err) {
		memcpy(ifr.lkl_ifr_hwaddr.sa_data, addr, LKL_ETH_ALEN);
		err = lkl_sys_ioctl(sock, LKL_SIOCSIFHWADDR, (__lkl_long_t)&ifr);
	}
...
int lkl_netdev_get_ifindex(int id)
{
	...
	ret = lkl_sys_ioctl(sock, LKL_SIOCGIFINDEX, (__lkl_long_t)&ifr);
	lkl_sys_close(sock);
