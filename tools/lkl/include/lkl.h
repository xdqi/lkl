...
{
	lkl_long_t p[6] = {LKL_CLOCK_MONOTONIC, 0, (lkl_long_t)rqtp, (lkl_long_t)rmtp, 0};
...
{
	long long res;
	lkl_long_t ret = lkl_sys_llseek(fd, off >> 32, off & 0xffffffff, &res, whence);
...
static inline void *lkl_sys_mmap(void *addr, size_t length, int prot, int flags,
				 int fd, off_t offset)
{
	return (void *)lkl_sys_mmap_pgoff((lkl_long_t)addr, length, prot, flags, fd,
					  offset >> 12);
}
 ...
 */
static inline lkl_long_t lkl_sys_open(const char *file, int flags, int mode)
{
 ...
 */
static inline lkl_long_t lkl_sys_creat(const char *file, int mode)
{
 ...
 */
static inline lkl_long_t lkl_sys_access(const char *file, int mode)
{
 ...
 */
static inline lkl_long_t lkl_sys_chown(const char *path, lkl_uid_t uid, lkl_gid_t gid)
{
 ...
 */
static inline lkl_long_t lkl_sys_chmod(const char *path, mode_t mode)
{
 ...
 */
static inline lkl_long_t lkl_sys_unlink(const char *path)
{
 ...
 */
static inline lkl_long_t lkl_sys_readlink(const char *path, char *buf, size_t bufsize)
{
 ...
 */
static inline lkl_long_t lkl_sys_mkdir(const char *path, mode_t mode)
{
 ...
 */
static inline lkl_long_t lkl_sys_rmdir(const char *path)
{
 ...
 */
static inline lkl_long_t lkl_sys_mknod(const char *path, mode_t mode, dev_t dev)
{
 ...
 */
static inline lkl_long_t lkl_sys_pipe(int fd[2])
{
 ...
 */
static inline lkl_long_t lkl_sys_send(int fd, void *buf, size_t len, int flags)
{
 ...
 */
static inline lkl_long_t lkl_sys_recv(int fd, void *buf, size_t len, int flags)
{
 ...
 */
static inline lkl_long_t lkl_sys_select(int n, lkl_fd_set *rfds, lkl_fd_set *wfds,
				  lkl_fd_set *efds, struct lkl_timeval *tv)
{
	lkl_long_t data[2] = { 0, _LKL_NSIG/8 };
 ...
 */
static inline lkl_long_t lkl_sys_poll(struct lkl_pollfd *fds, int n, int timeout)
{
	struct __lkl__kernel_timespec ts;
 ...
 */
static inline lkl_long_t lkl_sys_epoll_create(int size)
{
 ...
 */
static inline lkl_long_t lkl_sys_epoll_wait(int fd, struct lkl_epoll_event *ev,
				      int cnt, int to)
{
 ...
 * @returns - 0 on success, a negative value on error
 */
lkl_long_t lkl_mount_dev(unsigned int disk_id, unsigned int part, const char *fs_type,
